...
export class AdminService {
  ...
  update(admin: IAdmin): Observable<EntityResponseType> {
    return this.http.put<IAdmin>(`${this.resourceUrl}/${getAdminIdentifier(admin) as number}`, admin, { observe: 'response' });
  }

  partialUpdate(admin: IAdmin): Observable<EntityResponseType> {
    return this.http.patch<IAdmin>(`${this.resourceUrl}/${getAdminIdentifier(admin) as number}`, admin, { observe: 'response' });
  }
