...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query(
        value = "select distinct customer from Customer customer left join fetch customer.user left join fetch customer.tipoDocumento",
    ...
    @Query("select distinct customer from Customer customer left join fetch customer.user left join fetch customer.tipoDocumento")
    List<Customer> findAllWithToOneRelationships();

    @Query(
        "select customer from Customer customer left join fetch customer.user left join fetch customer.tipoDocumento where customer.id =:id"
    )
