...
public interface DocenteRepository extends JpaRepository<Docente, Long> {
    ...
    @Query(
        value = "select distinct docente from Docente docente left join fetch docente.customer left join fetch docente.centroDeFormacion",
    ...
    @Query("select distinct docente from Docente docente left join fetch docente.customer left join fetch docente.centroDeFormacion")
    List<Docente> findAllWithToOneRelationships();

    @Query(
        "select docente from Docente docente left join fetch docente.customer left join fetch docente.centroDeFormacion where docente.id =:id"
    )
